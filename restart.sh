...
force_restart() {
  ...
  if curl -s --connect-timeout 5 -o /dev/null -w '%{http_code}' http://localhost:$PORT 2>/dev/null | grep -q "200"; then
    echo "[restart] Server is up (port $PORT)"
  else
    echo "[restart] WARNING: server not responding after start"
  fi
}
...
if ! curl -s --connect-timeout 3 -o /dev/null -w '%{http_code}' http://localhost:$PORT 2>/dev/null | grep -q "200"; then
  force_restart
fi
